class ConsentManager {
  ...
  showConsentBanner() {
    ...
    // Get position from data attribute or default to bottom
    const position =
      document.querySelector("html").dataset.cookiePosition || "bottom";
  ...
  saveConsent(categories) {
    ...
    // Check if we're on a page with the consent data shortcode and reload if needed
  ...
  sendConsentToServer(consentData) {
    ...
    if (!window.customCookieConsent?.ajaxurl) {
      this.debugLog("No AJAX URL found for consent storage");
      return;
    }

    this.debugLog("Sending consent data to server:", {
      ajaxUrl: window.customCookieConsent.ajaxurl,
      nonceAvailable: !!window.customCookieConsent?.nonce,
      consentData: consentData,
    });

    // Create form data
    const formData = new FormData();
    formData.append("action", "custom_cookie_save_consent");
    formData.append("nonce", window.customCookieConsent?.nonce || "");
    formData.append("consent_data", JSON.stringify(consentData));

    // Send the data
    fetch(window.customCookieConsent.ajaxurl, {
      method: "POST",
